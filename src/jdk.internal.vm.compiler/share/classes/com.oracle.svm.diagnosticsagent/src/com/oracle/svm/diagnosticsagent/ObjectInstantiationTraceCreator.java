...
public class ObjectInstantiationTraceCreator extends JavaStackTraceCreator {
    ...
    public ObjectInstantiationTraceCreator(JvmtiEnv jvmti, JNIEnvironment jni) {
        super(jvmti, jni);
    ...
    protected void inspectStackTraceElementMethod(JNIMethodId jMethodID) {
        NativeImageDiagnosticsAgent agent = NativeImageDiagnosticsAgent.singleton();
        if (jMethodID.equal(agent.handles().getReportObjectInstantiatedMethodId())) {
            encounteredObjectInstantiatedReportCall = true;
        }
    }

    public boolean encounteredObjectInstantiatedReportCall() {
        return encounteredObjectInstantiatedReportCall;
    }
}
