...
import jdk.internal.vm.compiler.nativeimage.RuntimeOptions.OptionClass;
...
import jdk.internal.vm.compiler.options.OptionType;
...
class RuntimeOptionsSupportImpl implements RuntimeOptionsSupport {
    ...
    public OptionDescriptors getOptions(EnumSet<OptionClass> classes) {
        ...
        List<jdk.internal.vm.compiler.options.OptionDescriptor> graalvmDescriptors = new ArrayList<>();
        for (OptionDescriptor descriptor : descriptors) {
            if (classes.contains(getOptionClass(descriptor))) {
                jdk.internal.vm.compiler.options.OptionDescriptor.Builder builder = jdk.internal.vm.compiler.options.OptionDescriptor.newBuilder(asGraalVMOptionKey(descriptor), descriptor.getName());
    ...
    private static <T> jdk.internal.vm.compiler.options.OptionKey<T> asGraalVMOptionKey(OptionDescriptor descriptor) {
        ...
        return new jdk.internal.vm.compiler.options.OptionKey<>(defaultValue, type);
    }
